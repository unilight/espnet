...
if [ ${vocoder} != "None" ]; then
    echo "Evaluate: VC model"

    # select vocoder type (GL, WNV, PWG)
    if [ ${vocoder} == "WNV" ]; then
        wavdir=${outdir}_denorm/${pair_name}/wnv_wav_nsf
    elif [ ${vocoder} == "PWG" ]; then
        wavdir=${outdir}_denorm/${pair_name}/pwg_wav
    elif [ ${vocoder} == "GL" ]; then
        wavdir=${outdir}_denorm/${pair_name}/wav
    else
    ...
    mkdir -p ${outdir}_denorm/${trg_name}/wav
    cat < data/${trg_name}/wav.scp | awk '{print $1}' | while read -r filename; do
        if [ -L ${outdir}_denorm/${trg_name}/wav/${filename}.wav ]; then
            unlink ${outdir}_denorm/${trg_name}/wav/${filename}.wav
        fi
        newname=$(echo "$filename" | sed -e "s/"$trgspk"_//g")
        cp ${db_root}/cmu_us_${trgspk}_arctic/wav/${newname}.wav ${outdir}_denorm/${trg_name}/wav/${filename}.wav
    done
    
    wavdir=${outdir}_denorm/${trg_name}/wav
    name=$trg_name
fi
