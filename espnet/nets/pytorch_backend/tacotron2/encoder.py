...
class EncoderNoEmb(torch.nn.Module):
    """Encoder module of Spectrogram prediction network, without embedding LUT.

    This is a "modified" module of encoder of Spectrogram prediction network in Tacotron2, which described in `Natural TTS
    Synthesis by Conditioning WaveNet on Mel Spectrogram Predictions`_. This is the encoder which converts the
    "spectrogram" into the sequence of hidden states.
    ...
    def __init__(self, idim,
        ...
        if econv_layers > 0:
            self.convs = torch.nn.ModuleList()
            for layer in six.moves.range(econv_layers):
                ichans = idim if layer == 0 else econv_chans
                if use_batch_norm:
                    self.convs += [torch.nn.Sequential(
                        torch.nn.Conv1d(ichans, econv_chans, econv_filts, stride=1,
                                        padding=(econv_filts - 1) // 2, bias=False),
            ...
            self.convs = None
        if elayers > 0:
            iunits = econv_chans if econv_layers != 0 else idim
            self.blstm = torch.nn.LSTM(
                iunits, eunits // 2, elayers,
                batch_first=True,
                bidirectional=True)
    ...
    def forward(self, xs, ilens=None):
        ...
        Args:
            xs (Tensor): Batch of the padded spectrogram (B, Tmax, idim). Padded value should be 0.
            ilens (LongTensor): Batch of lengths of each input batch (B,).
